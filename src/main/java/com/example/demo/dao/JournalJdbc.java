...
public class JournalJdbc
{
    ...
    {
        return jdbcTemplate.query("SELECT JOURNAL.*, SUM(m.value) FROM JOURNAL INNER JOIN MARK m ON m.id = JOURNAL.mark_id GROUP BY JOURNAL.student_ id ORDER BY SUM(m.value) ", this::mapJournal);
    }
