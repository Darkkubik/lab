...
public class JournalJdbc
{
    ...
    // ПРОСМОТР ЖУРНАЛА ORDER
    public List<Journal> get_all_order()
    {
        return jdbcTemplate.query("SELECT STUDENT_ID, s.SECOND_NAME, s.NAME, s.SURNAME FROM JOURNAL INNER JOIN MARK m ON JOURNAL.mark_id = m.id INNER JOIN STUDENT s ON JOURNAL.STUDENT_ID = s.ID GROUP BY JOURNAL.STUDENT_ID ORDER BY SUM(m.value) DESC;", this::mapJournal);
    }
